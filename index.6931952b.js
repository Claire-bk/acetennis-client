...
window.onload = ()=>{
    ...
    if (localStorage.getItem("PlayerId")) displayJoinMatch("cancel");
    else displayJoinMatch("join");
    ...
    } else document.querySelector(".icon__logout").classList.add("invisible");
};
...
// set days
if (fullYear % 4 == 0 && fullYear % 100 != 0 || fullYear % 400 == 0) // leap year setting
lastDayOfMonth[1] = 29;
const days = document.querySelector(".days");
for(let i = 1; i <= lastDayOfMonth[monthNum]; i++)if (i === today) days.innerHTML += `<li class="day" data-day="${i}"><span class="active">${i}</span></li>`;
else days.innerHTML += `<li class="day" data-day="${i}">${i}</li>`;
days.addEventListener("click", (event)=>{
    const date1 = event.target.dataset.match;
    if (!date1) return;
    // previous match
    fetchMatch(parseInt(date1), today);
});
function fetchMatch(date2, today1) {
    const fullDate = `${fullYear}-${currMonth + 1}-${date2}`;
    const fetchEvent = `${baseURL}/event?date=${fullDate}`;
    const fetchMatch1 = `${baseURL}/matches?date=${fullDate}`;
    today1 - date2 > 0 ? fetchMatchResult(fetchMatch1) : fetchEventInfo(fetchEvent);
}
...
function displayMatchResult(result) {
    ...
    for(let i1 = 0; i1 < result.length; i1++)matchInfo.innerHTML += `<div class="match-info__result">
            <h3>Court : ${result[i1].courtNum}</h3>
            <p>${result[i1].playerA1}  :  ${result[i1].playerB1}</p>
            <p>${result[i1].playerA2}  :  ${result[i1].playerB2}</p>
            <p>${result[i1].scoreA} : ${result[i1].scoreB}</p>
            </div>`;
}
...
function getJoinMatchInfo() {
    ...
    if (!username) return;
    const date3 = localStorage.getItem("UpcomingMatch");
    if (!date3) return;
    fetch(`${baseURL}/players?username=${username}&date=${date3}`, {
        method: "GET",
        headers: {
            "content-Type": "application/json"
        }
...
getJoinMatchInfo();
function getFormatDate(date4) {
    let month1 = (date4.getMonth() + 1).toString();
    let day = date4.getDate().toString();
...
function findUpcomingDate(matches) {
    ...
    for(let i2 = 0; i2 < matches.length; i2++){
        const matchDate = new Date(matches[i2]["date"]);
        const timeDiff = matchDate - today2;
        if (timeDiff > 1) {
            const hours = Math.floor(timeDiff / 1000 / 60 / 60);
            if (hours > 1 && hours < 168) return matchDate;
        }
    }
    return undefined;
}
function displayMatchDays(matches) {
    for(let i3 = 0; i3 < matches.length; i3++){
        const dateOfMonth = new Date(matches[i3]["date"]);
        const date5 = dateOfMonth.getDate();
        for(let j = 1; j <= lastDayOfMonth[monthNum]; j++){
            const day = document.querySelector(`.day[data-day="${j}"]`);
            if (day.dataset.day === date5.toString()) day.innerHTML = `<span class="matches" data-match=${day.dataset.day}>${j}</span>`;
        }
    }
}
function getHeaders() {
...
//# sourceMappingURL=index.6931952b.js.map
